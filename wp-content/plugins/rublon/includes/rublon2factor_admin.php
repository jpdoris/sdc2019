...
function rublon2factor_add_menu_entries()
{
    ...
    add_menu_page($main_title, $main_title, 'read', 'rublon' , 'rublon2factor_render_settings_page', 'div');
...
function rublon2factor_register_settings()
{
    ...
    add_settings_field('rublon2factor_api_credentials', __('Rublon Admin Console', 'rublon'), 'rublon2factor_render_api_credentials', 'rublon_api', 'rublon2factor-settings');
    ...
    add_settings_field('rublon2factor_display_rublon_badge', __('Rublon Badge', 'rublon'), 'rublon2factor_render_display_rublon_badge', 'rublon_api', 'rublon2factor-settings');
    ...
    <div class="wrap">
        <div id="rublon2factor_page">
            <h2 class="rublon-header">
                <?php _e('Rublon Two-Factor Authentication Settings', 'rublon'); ?>
            </h2>
            ...
            // Header text
            echo '<p>'
                . ' ' . sprintf(__('Learn more at <a href="%s" target="_blank">www.rublon.com</a>. Contact support at <a href="mailto:%s">support@rublon.com</a>.', 'rublon'), RublonHelper::rublonComURL(), RublonHelper::RUBLON_EMAIL_SUPPORT)
                . '</p>';
...
function rublon2factor_add_userlist_columns($columns)
{
    ...
    if (RublonHelper::isSiteRegistered()) {
        ...
        foreach ($columns as $k => $v) {
            $new_columns[$k] = $v;
            if ($k == 'username') {
                $new_columns['rublon2factor_status'] = __('Rublon 2FA', 'rublon');
            }
        }
        return $new_columns;
    } else {
...
function rublon2factor_render_display_rublon_badge()
{
    echo RublonHelper::renderRublonBadgeForm();
}