...
use Rublon_WordPress\Libs\RublonImplemented\RublonAPINotify;
...
class RublonHelper
{
    ...
    static private function _performUpdate($from)
    {
		...
		// clear old API credentials
        self::_clearOldApiCredentials();
    }
    ...
    static public function notify($error_info, $options)
    {
        if (self::isTrackingAllowed()) {
            ...
            $notify = new RublonAPINotify(self::getRublon(), self::getUserId(), json_encode($error_info));
            try {
                $notify->perform();
            } catch (RublonException $e) {
                $notify = null;
...
%s
     ...
     * @return bool
     */
    static public function getUserProtectionStatus($user)
    {
        $rublon_req = new RublonRequests();
        $mobile_user_status = $rublon_req->checkMobileStatus($user);
        self::setMobileUserStatus($user, $mobile_user_status);
     ...
     * @param WP_User $user
     */
    static public function printProfileSectionAdditions($user, $current = true)
    {
        $role_protection_type = self::roleProtectionType($user);
        $user_protection_type = self::userProtectionType($user);
        $mobile_user_status = self::getMobileUserStatus($user, true);
        ...
        <h3 class="rublon-header">
            <?php _e('Rublon Two-Factor Authentication', 'rublon'); ?>
        </h3>
        <table class="form-table">
            <tr>
                <th>
                    <?php _e('Protection', 'rublon'); ?>
                </th>
                <td>
                    ...
                    <?php if ($current) { ?>
                        ...
                        if ($role_protection_type == self::PROTECTION_TYPE_MOBILE || $mobile_user_status == self::YES):
                            ...
                            <div class="rublon-description">
                                <?php
                                echo __('Your account is protected by Rublon Two-Factor Authentication.', 'rublon');
                                ?>
                            </div>
                        <?php
                        elseif ($role_protection_type == self::PROTECTION_TYPE_EMAIL):
				...
				<span class="description"><?php
                    echo __('You will need to confirm every sign in from a new device via email. You will also need to confirm changes to important settings like your password or email address.', 'rublon')
                        ...
                        <div class="rublon-description">
                            <?php
                            if (RublonHelper::getMobileUserStatus($user)) {
                                echo '<table><tr><td style="padding: 0"><img width="40px" src="' . RUBLON2FACTOR_PLUGIN_URL . '/assets/images/toggle-on.png' . '" align="center"></td><td>' . __('On', 'rublon') . '</td></tr></table>';
                                echo sprintf(__('To disable protection for this user, please go to the Rublon Admin Console at <a href="%s" target="_blank">%s</a>.', 'rublon'), RUBLON_ADMIN_URL, RUBLON_ADMIN_URL);
                            } else {
                                echo '<table><tr><td style="padding: 0"><img width="40px" src="' . RUBLON2FACTOR_PLUGIN_URL . '/assets/images/toggle-off.png' . '" align="center"></td><td>' . __('Off', 'rublon') . '</td></tr></table>';
                                echo sprintf(__('To enable protection for this user, please go to the Rublon Admin Console at <a href="%s" target="_blank">%s</a>.', 'rublon'), RUBLON_ADMIN_URL, RUBLON_ADMIN_URL);
                            }
                            ?>
                        </div>
                        <?php
...
//	        AND $additional_settings[RublonHelper::RUBLON_SETTINGS_ADAM] == 'on');
    ...
    static public function renderApiCredentialsForm()
    {
        ...
        $status = RublonHelper::isSiteRegistered() ?
            '<span class="rublon-green rublon-bold">' . __('Connected', 'rublon') . '</span> - ' . sprintf(__('This application is connected to an organization. Go to the Rublon Admin Console at <a href="%s" target="_blank">%s</a> to manage its settings.', 'rublon'),RUBLON_ADMIN_URL,RUBLON_ADMIN_URL)
            :
            '<span class="rublon-red rublon-bold">' . __('Not connected', 'rublon') . '</span> - ' . sprintf(__('This application is not connected to any organization. Enter the System Token and Secret Key below to establish a connection (both values can be fetched from the Rublon Admin Console at <a href="%s" target="_blank">%s</a>).', 'rublon'), RUBLON_ADMIN_URL, RUBLON_ADMIN_URL);
        $html = '<p class="rublon-settings-desc">' . $status . '</p>';
        $html .= '<table class="form-table">                    
                    ...
                    <th>' . __('Secret Key', 'rublon') . '</th>
                       <td>                                                                 
                                <input type="text" name="' . RublonHelper::RUBLON_SETTINGS_KEY . '[rublon_secret_key]" id="rublon-secret-key"
    ...
    static public function renderRublonBadgeForm()
    {
        $options = RublonHelper::getSettings();

        $displayRublonBadge = !empty($options['display_rublon_badge']) ? $options['display_rublon_badge'] : false;
        $html = '<p class="rublon-settings-desc">' . __('Display Rublon Badge on login form (wp-login.php)', 'rublon') . '</p>';
        $html .= '<input type="hidden" name="' . RublonHelper::RUBLON_SETTINGS_KEY . '[display_rublon_badge]" value="0"/>
                  <input type="checkbox" name="' . RublonHelper::RUBLON_SETTINGS_KEY . '[display_rublon_badge]" id="display-rublon-badge" ' . (!empty($displayRublonBadge) ? 'checked="checked"' : '') . '/>';
	...
	static function _clearOldApiCredentials() {
        $settings = RublonHelper::getSettings();
        $settings['rublon_system_token'] = '';
