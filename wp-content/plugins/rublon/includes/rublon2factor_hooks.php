            ...
            if (!empty($users)) {
                ...
                foreach ($users as $user) {
                    if ($user instanceof WP_User) {
                        $user_id = RublonHelper::getUserId($user);
                        $mobile_user_status = RublonHelper::getUserProtectionStatus($user);
                ...
                foreach ($requested_users as $user) {
                    ...
                    if (!empty($check) && /*$check->isProtectionEnabled($user_id)*/
                        $check->isUserProtected($user->user_email)) {
...
{
    $settings = RublonHelper::getSettings();
    if (!empty($settings['display_rublon_badge'])) {
        ...
        if (!RublonHelper::isNewVersion()) {
            echo '(function (d, s, id) { var js; if (d.getElementById(id)) return; js = d.createElement(s); js.id = id; js.src = "' . RublonHelper::getAPIDomain() . '/rublon-sdk-js-sa"; var p = d.getElementsByTagName(s)[0]; p.parentNode.insertBefore(js, p); })(document, "script", "RublonConsumerJs-sdk");';
        }
        echo '    
            if (RublonWP) {
    			RublonWP.showBadge();
...
function rublon2factor_edit_user_profile($profileUser)
{
    if (RublonHelper::isSiteRegistered()) {
        RublonHelper::printProfileSectionAdditions($profileUser, false);
    }
}

add_action('edit_user_profile', 'rublon2factor_edit_user_profile');