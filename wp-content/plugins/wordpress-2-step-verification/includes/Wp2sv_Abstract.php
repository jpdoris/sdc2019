...
class Wp2sv_Abstract
{
    ...
    function loadTemplate($_name,$data=[]){
        $_name=str_replace('.','/',$_name);
        $extensions=['.php'];
        $templates=[];
        if(defined('TEMPLATEPATH')){
            $templates[]=TEMPLATEPATH.'/wp2sv/'.$_name;
        }
        $templates[]=WP2SV_TEMPLATE.'/'.$_name;
        extract($data,EXTR_SKIP);
        foreach($templates as $template_file_name){
            foreach ($extensions as $extension) {
                $template_file=$template_file_name.$extension;
